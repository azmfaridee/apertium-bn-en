...
cat /tmp/fresh.adj | grep -v -w -f /tmp/adjs | sed 's/$/,\!/g' | cat  >> ../adjective/fresh.adj

cat ../noun/noun.bn.dix | grep "<e.*lm" | sed 's/.*<i>//g' | sed  's/<\/i>.*//g' > /tmp/nouns
cat ../noun/fresh.noun | sed 's/,/\t/g' | cut -f1 | cat >> /tmp/nouns
cat /tmp/fresh.noun | grep -v -w -f /tmp/nouns | sed 's/$/,\!/g' | cat  >> ../noun/fresh.noun
...
cat /tmp/fresh.proper-noun | grep -v -w -f /tmp/proper-nouns | sed 's/$/,\!/g' | cat  >> ../proper-noun/fresh.proper-noun
